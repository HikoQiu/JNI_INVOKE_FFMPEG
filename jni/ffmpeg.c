...
int main(int argc, char **argv)
{
    ...
    if (ret < 0) {
    	//exit_program(1);
    	main_return_code = 1001; // 解析参数失败
    	return main_return_code;
    }

    // log
	LOGI("finished parse options.");

    if (nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
        av_log(NULL, AV_LOG_WARNING, "Use -h to get full help or, even better, run 'man %s'\n", program_name);
        //exit_program(1);
        main_return_code = 1011; // 没有输入和输出的文件
        return main_return_code;
    }
    ...
    if (nb_output_files <= 0) {
        ...
        //exit_program(1);
        main_return_code = 1012; // 输出的文件参数不合法
        return main_return_code;
    }
...
//     }
    ...
    if (transcode() < 0) {
    	//exit_program(1);
    	main_return_code = 1021; // transcode小于0
    	return main_return_code;
    }
    ...
    if ((decode_error_stat[0] + decode_error_stat[1]) * max_error_rate < decode_error_stat[1]) {
    	//exit_program(69);
    	main_return_code = 1031; // 处理出错
    	return main_return_code;
    }


    //exit_program(received_nb_signals ? 255 : main_return_code);
    // 1041 ： received_nb_signals
    return received_nb_signals ? 1041 : main_return_code;
}
